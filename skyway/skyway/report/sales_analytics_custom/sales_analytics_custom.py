...
class Analytics(object):
	...
	def get_sales_transactions_based_on_items(self):
		...
		self.entries = frappe.db.sql("""
			...
			where s.name = i.parent and i.docstatus = 1 and s.company = %s and i.item_code = {item}
			and s.{date_field} between %s and %s
		"""
		.format(date_field=self.date_field, value_field=value_field, doctype=self.filters.doc_type, item=self.filters.item_code),
		(self.filters.company, self.filters.from_date, self.filters.to_date), as_dict=1)
